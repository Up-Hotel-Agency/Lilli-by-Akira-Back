...
if( get_offer_single_type() == 'offer_2' ) {
	// add categories for offer listing 2
	function offers_taxonomies() {
		register_taxonomy('offers-categories', array('offers'), array(
			'labels' => array(
				'name' => 'Offers Categories'
			),
			'show_ui' => true,
			'show_tagcloud' => false,
			'rewrite' => array('with_front' => false, 'slug' => 'offers')
		));
	}
	add_action('init', 'offers_taxonomies');
}