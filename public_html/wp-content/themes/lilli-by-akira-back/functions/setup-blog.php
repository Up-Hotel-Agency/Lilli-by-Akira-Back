...
function ajax_load_more() {
    if ( isset($_REQUEST) ):
        ...
        $args = array( 'post_type' => 'post', 'posts_per_page' => 9, 'offset' => $offset, 'post_status' => 'publish' );
...
// Function Add event to Calendar
function add_calendar(){
    ?>
    <div class="add-to-calendar">
        <span>Add to Calendar</span>
        <?php $title = get_the_title(); ?>
        <?php $description = get_field('intro_content', false, false);?>
        <?php $startDate = date_format( date_create(get_field('date')), "Ymd"); ?>
        <?php $startDateForm = date_format( date_create(get_field('date')), "Y-m-d"); ?>
        <?php if(get_field('date_range')): ?>
            <?php $endDate = date_format( date_create(get_field('end_date')), "Ymd"); ?>
            <?php $GoogleendDate = date('Ymd', strtotime($endDate . ' +1 day')); ?>
            <?php $endDateForm = date_format( date_create(get_field('end_date')), "Y-m-d"); ?>
        <?php else: ?>
            <?php $endDate = date_format( date_create(get_field('date')), "Ymd"); ?>
            <?php $GoogleendDate = date('Ymd', strtotime($endDate . ' +1 day')); ?>
            <?php $endDateForm = date_format( date_create(get_field('date')), "Y-m-d"); ?>
        <?php endif; ?>
        <?php $location = get_field('location');?>
        <div class="calendar-list">
            <a rel="noopener" target="_blank" href="https://calendar.google.com/calendar/render?action=TEMPLATE&text=<?php echo $title; ?>&details=<?php echo $description; ?>&dates=<?php echo $startDate; ?>/<?php echo $GoogleendDate; ?>&location=<?php echo $location; ?>">Google</a>
            <a rel="noopener" target="_blank" href="https://outlook.office.com/calendar/deeplink/compose?subject=<?php echo $title; ?>&body=<?php echo $description; ?>&startdt=<?php echo $startDateForm; ?>&enddt=<?php echo $endDateForm; ?>&location=<?php echo $location; ?>&path=%2Fcalendar%2Faction%2Fcompose&rru=addevent">Outlook</a>
            <a rel="noopener" target="_blank" href="https://calendar.yahoo.com/?desc=<?php echo $description; ?>&st=<?php echo $startDate; ?>&et=<?php echo $endDate; ?>&in_loc=<?php echo $location; ?>&title=<?php echo $title; ?>&v=60">Yahoo</a>
        </div>
    </div>
    <?php 
}