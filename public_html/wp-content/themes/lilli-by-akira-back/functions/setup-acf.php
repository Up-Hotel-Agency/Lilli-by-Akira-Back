...
function acf_google_map_api( $api ){
    ...
    if(preg_match('/(up-dev)/',$hostName) || preg_match('/(adaodev)/',$hostName)){
        $api['key'] = 'AIzaSyCZkamJZ4UDWjH4BNNN8JshSWbF6vpaIrM';
    }else{
        $api['key'] = 'AIzaSyCZkamJZ4UDWjH4BNNN8JshSWbF6vpaIrM';
    }
    return $api;
...
function linkField($field, $function){
    ...
    if( $function == 'icon_position') {
        if( $field['icon_position'] != 'none' ) {
            return $field['icon_position'];
        }
    }
    if( $function == 'icon') {
        if( $field['icon_position'] != 'none' ) {
            return $field['autoloaded_icon'];
        }
    }
}
